bl_info = {
    "name": "UAV Export",
    "author": "magicLab, Bassam Kurdali",
    "version": (0, 3),
    "blender": (2, 78, 0),
...
class ExportCSVLocations(Operator, ExportHelper):
    ...
    def execute(self, context):
            ...
            for fc in fcurves:
                for keyframe in fc.keyframe_points:
                    ...
                    if frame >= start_frame and frame <= end_frame:
                        frames.add(keyframe.co[0])  # Only add in range
            return frames
        ...
        for ob in obs:
            ...
            ob_frames[ob.name] = sorted(frames) 
            total_frames = total_frames.union(frames)
        total_frames = sorted(total_frames)
        for frame in total_frames:
            scene.frame_set(frame) # slow but robust
            for ob in obs:
                if frame in ob_frames[ob.name]:
                    ...
                    ob_frames[ob.name][index] = (
                        ...
                        ob.location[1],
                        ob.location[2],
                        (frame - start_frame) / frames_per_second)
        with open(self.filepath, 'w', newline='') as csvfile:
            uavwriter = csv.writer(
                csvfile, delimiter =',', quoting=csv.QUOTE_MINIMAL)
            uavwriter.writerow(["id", "x[m]", "y[m]", "z[m]", "t[s]"])
